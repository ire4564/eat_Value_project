...
class MakeRoom extends Component {
    ...
    readClipboard = async () => {
        ...
        if (linkAddress.indexOf("http") > - 1) {
            ...
            Alert.alert("아래 공유 링크를 등록하시겠어요?", linkAddress, [
                {
                    text: "아니요",
                    style: "cancel"
                }, {
                    text: "네",
                    onPress: () => this.setState({linkAddress: linkAddress})
                }
            ], {cancelable: false});
        } else {
            Alert.alert(
                '클립보드에 공유 링크를 복사하지 않으셨네요!',
                '주문하시려고 하는 배달 어플리케이션에서 [공유하기] 기능을 이용해 가게 정보를 클립보드에 복사해주세요. 다른 사용자들의 메뉴 선택에 도움이 됩니다.',
                [
                    {
                        text: "알겠습니다"
                    }
    ...
    _get() {
        ...
        fetch(`${databaseURL}/db_store_menu/0`)
            .then(res => {
                if (res.status != 200) {
                    throw new Error(res.statusText);
                }
    ...
    _post(jsondata) {
        return fetch(
            `${databaseURL}/db_order/${this._get_date()}`,
            { // TODO : set table json name
                method: 'POST',
                headers: {
    ...
    createRoom = () => {
        this.setState({totalPrize: this.renew_prize()})
        ...
        if (order_details[i].user_id == this.state.db_user) {
            ...
            list_info.push(
                <View key={"info" + i}>
                    ...
                    <Text style={styles.detail_prize}>● 기본: {this.addComma(order_details[i].price)}
                        원</Text>
                    <Text style={styles.detail_prize}>
                        {this.addComma(this_total_prize)}
                        원</Text>
                    <OrderItem
                        //그 카운터의 수를 세기
                        btnNum={i} num={order_details[i].amount} sendCounter={this
                            .sendCounter
                            .bind(this)} countPrize={true}/>
    ...
    var info = <View>
        ...
        <View style={styles.togetPrize}>
            <Text style={styles.search}>
                {this.addComma(this.state.db_store.min_order)}
                원
            </Text>
        </View>
...
sendCounter(_data, index, type) {
    var detail = this
        ...
        .order_detail[index]
        .amount = _data
    this.setState({db_order: detail, clickType: type, BtnIndex: index});
}
            ...
            <ScrollView>
                <View style={styles.main}>
                    <Text style={styles.headline}>
                        <AntDesign name="checksquare" size={20} color={ICON_COLOR} style={styles.icon}/>
